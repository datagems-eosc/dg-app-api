...
namespace DataGEMS.Gateway.App.Model.Builder
{
	public class DatasetBuilder : PrimitiveBuilder<Model.Dataset, Service.DataManagement.Model.Dataset>
	{
		...
		public override async Task<List<Model.Dataset>> Build(IFieldSet fields, IEnumerable<Service.DataManagement.Model.Dataset> datas)
		{
			this._logger.Debug(new MapLogEntry("building").And("type", nameof(Model.Dataset)).And("fields", fields).And("dataCount", datas?.Count()));
			...
			foreach(Service.DataManagement.Model.Dataset d in datas ?? Enumerable.Empty<Service.DataManagement.Model.Dataset>())
			{
				...
				if (fields.HasField(nameof(Model.Dataset.Features))) m.Features = features.ContainsKey(d.Id) ? features[d.Id] : null;
		...
		private async Task<Dictionary<Guid, List<Model.Collection>>> CollectCollections(IFieldSet fields, IEnumerable<Service.DataManagement.Model.Dataset> datas)
		{
