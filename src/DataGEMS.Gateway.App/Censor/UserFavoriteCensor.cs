...
namespace DataGEMS.Gateway.App.Censor
{
	public class UserFavoriteCensor : ICensor
	{
		...
		private readonly ILogger<UserFavoriteCensor> _logger;
		...
		public UserFavoriteCensor(
			CensorFactory censorFactory,
			IAuthorizationService authService,
			ILogger<UserFavoriteCensor> logger,
		...
		{
			this._logger.Debug(new MapLogEntry("censoring").And("type", nameof(Model.UserFavorite)).And("fields", fields).And("context", context).And("userId", userId));
			...
			censored = censored.MergeAsPrefixed(await this._censorFactory.Censor<UserCensor>().Censor(fields.ExtractPrefixed(nameof(Model.UserFavorite.User).AsIndexerPrefix()), context, userId), nameof(Model.UserFavorite.User));
			censored = censored.MergeAsPrefixed(await this._censorFactory.Censor<DatasetCensor>().Censor(fields.ExtractPrefixed(nameof(Model.UserFavorite.Dataset).AsIndexerPrefix()), context), nameof(Model.UserFavorite.Dataset));
