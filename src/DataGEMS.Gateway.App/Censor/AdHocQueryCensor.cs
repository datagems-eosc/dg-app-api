...
{
	public class AdHocQueryCensor : ICensor
	{
		private readonly CensorFactory _censorFactory;
		private readonly IAuthorizationService _authService;
		private readonly ILogger<AdHocQueryCensor> _logger;
		...
		public AdHocQueryCensor(
			CensorFactory censorFactory,
			IAuthorizationService authService,
			ILogger<AdHocQueryCensor> logger,
		...
		{
			this._logger.Debug(new MapLogEntry("censoring").And("type", nameof(Model.AdHocQuery)).And("fields", fields).And("context", context).And("userId", userId));
			...
			{
				case CensorBehavior.Censor: { authZPass = await this._authService.AuthorizeOrOwner(!String.IsNullOrEmpty(subjectId) ? new OwnedResource(subjectId) : null, Permission.BrowseAdHocQuery); break; }
				case CensorBehavior.Throw:
				default: { authZPass = await this._authService.AuthorizeOrOwnerForce(!String.IsNullOrEmpty(subjectId) ? new OwnedResource(subjectId) : null, Permission.BrowseAdHocQuery); break; }
			}
			if (authZPass)
			{
