...
{
	public class InDataExplorationCensor : ICensor
	{
		private readonly CensorFactory _censorFactory;
		private readonly IAuthorizationService _authService;
		private readonly ILogger<InDataExplorationCensor> _logger;
		...
		public InDataExplorationCensor(
			CensorFactory censorFactory,
			IAuthorizationService authService,
			ILogger<InDataExplorationCensor> logger,
		...
		{
			this._logger.Debug(new MapLogEntry("censoring").And("type", nameof(Model.InDataExplore)).And("fields", fields).And("context", context));
			...
			{
				case CensorBehavior.Censor: { authZPass = await this._authService.Authorize(Permission.CanExecuteInDataExploration); break; }
				case CensorBehavior.Throw:
				default: { authZPass = await this._authService.AuthorizeForce(Permission.CanExecuteInDataExploration); break; }
			}
			if (authZPass)
			{
