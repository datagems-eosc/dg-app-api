...
namespace DataGEMS.Gateway.App.Censor
{
	public class ConversationCensor : ICensor
	{
		...
		public async Task<IFieldSet> Censor(IFieldSet fields, CensorContext context, Guid? userId = null)
		{
			...
			censored = censored.MergeAsPrefixed(await this._censorFactory.Censor<ConversationDatasetCensor>().Censor(fields.ExtractPrefixed(nameof(Model.Conversation.Datasets).AsIndexerPrefix()), context, userId), nameof(Model.Conversation.Datasets));
			censored = censored.MergeAsPrefixed(await this._censorFactory.Censor<ConversationMessageCensor>().Censor(fields.ExtractPrefixed(nameof(Model.Conversation.Messages).AsIndexerPrefix()), context, userId), nameof(Model.Conversation.Messages));
