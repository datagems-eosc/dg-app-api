...
{
	public class PackageRecommenderCensor : ICensor
	{
		private readonly CensorFactory _censorFactory;
		private readonly IAuthorizationService _authService;
		private readonly ILogger<PackageRecommenderCensor> _logger;
		...
		public PackageRecommenderCensor(
			CensorFactory censorFactory,
			IAuthorizationService authService,
			ILogger<PackageRecommenderCensor> logger,
		...
		public async Task<IFieldSet> Censor(IFieldSet fields, CensorContext context)
		{
			...
			{
				case CensorBehavior.Censor: { authZPass = await this._authService.Authorize(Permission.CanRetrievePackage); break; }
				case CensorBehavior.Throw:
				default: { authZPass = await this._authService.AuthorizeForce(Permission.CanRetrievePackage); break; }
			}
			if (authZPass)
			{
