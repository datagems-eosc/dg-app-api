...
namespace DataGEMS.Gateway.App.Query
{
	public class WorkflowExecutionHttpQuery : Cite.Tools.Data.Query.IQuery
	{
		...
		public async Task<Service.Airflow.Model.AirflowDagExecution> ByIdAsync()
		{
			...
			this._logger.Debug("Sending request to {url}", $"{this._config.BaseUrl}{this._config.DagExecutionByIdEndpoint.Replace("{workflowId}", this._workflowIds[0]).Replace("{id}", this._id)}");
		...
		private async Task<Service.Airflow.Model.AirflowDagExecutionList> CollectBaseAsync(Boolean useInCount)
		{
			...
			this._logger.Debug("Sending request to {url} with body {payload}", $"{this._config.BaseUrl}{this._config.DagExecutionListEndpoint}", this._jsonHandlingService.ToJson(requestModel));
