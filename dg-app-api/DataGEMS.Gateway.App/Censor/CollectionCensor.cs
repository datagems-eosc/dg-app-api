...
{
	public class CollectionCensor : ICensor
	{
		private readonly CensorFactory _censorFactory;
		private readonly IAuthorizationService _authService;
		private readonly ILogger<CollectionCensor> _logger;
		...
		public CollectionCensor(
			CensorFactory censorFactory,
			IAuthorizationService authService,
			ILogger<CollectionCensor> logger,
		...
		public async Task<IFieldSet> Censor(IFieldSet fields, CensorContext context)
		{
			...
			{
				case CensorBehavior.Censor: { authZPass = await this._authService.AuthorizeOrAffiliatedDataset(new AffiliatedDatasetResource(userDatasetRoles), Permission.BrowseCollection); break; }
				case CensorBehavior.Throw:
				default: { authZPass = await this._authService.AuthorizeOrAffiliatedDatasetForce(new AffiliatedDatasetResource(userDatasetRoles), Permission.BrowseCollection); break; }
			}
			if (authZPass)
			{
